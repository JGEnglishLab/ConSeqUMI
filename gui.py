...
class longreadWindow(QWidget):
    def __init__(self):
        ...
        self.outputNameField.setText("delete/newConsensus")
        ...
        self.variantLabel.setToolTip(
            "A flag indicating if variants should be deduced from consensus sequences. \nFor example, if consensus sequences 1, 2, and 3 are generated, and sequences 1 and 3 are the same sequence, the variant file will combine them. \nThe variant output would then have 2 sequences."
        )
        self.variantCheckBox = QCheckBox()
        formLayout.addRow(self.variantLabel, self.variantCheckBox)

        self.benchmarkLabel = QLabel(
            "Perform Benchmarking Run (note: runtime significantly longer)"
        )
        self.benchmarkLabel.setToolTip(
            "A flag indicating we want to benchmark the optimal cluster size required to generate an accurate consensus sequence."
        )
        self.benchmarkCheckBox = QCheckBox()
        formLayout.addRow(self.benchmarkLabel, self.benchmarkCheckBox)
        self.benchmarkCheckBox.stateChanged.connect(
            lambda: self.disable_second_box(
                self.benchmarkCheckBox, self.variantCheckBox
            )
        )
        ...
        self.processCopyButton.clicked.connect(
            lambda: self.copy_to_clipboard(self.processText.toPlainText())
        )
    ...
    def set_args(self):
        ...
        tempDict["inputDir"] = self.return_file_path_value(
            self.inputField.text(), self.inputLabel
        )
        tempDict["outputDir"] = self.return_file_path_value(
            self.outputField.text(), self.outputLabel
        )
        ...
        tempDict["outputName"] = self.outputNameField.text()
        self.set_text_color(self.outputNameLabel, isValid=True)
        tempDict["adapterFile"] = self.return_file_path_value(
            self.adapterField.text(), self.adapterLabel, permittedTypes=["txt"]
        )
        tempDict["consensusAlgorithm"] = self.consensusAlgorithmComboBox.currentText()
        self.set_text_color(self.consensusAlgorithmLabel, isValid=True)
        if self.strictVLooseCheckBox.isChecked():
            tempDict["isLoose"] = 2
            self.set_text_color(self.strictVLooseLabel, isValid=True)
        else:
            tempDict["isLoose"] = 1
            self.set_text_color(self.strictVLooseLabel, isValid=True)
        if self.variantCheckBox.isChecked():
            tempDict["isVariant"] = 2
            self.set_text_color(self.variantLabel, isValid=True)
        else:
            tempDict["isVariant"] = 1
            self.set_text_color(self.variantLabel, isValid=True)
        if self.benchmarkCheckBox.isChecked():
            tempDict["isBenchmark"] = 2
            self.set_text_color(self.benchmarkLabel, isValid=True)
        else:
            tempDict["isBenchmark"] = 1
            self.set_text_color(self.benchmarkLabel, isValid=True)

        if False in list(tempDict.values()):
            return False
        args = []
        ...
        if tempDict["isVariant"] == 2:
            args += ["-v"]
        if tempDict["isBenchmark"] == 2:
            args += ["-bc"]
    ...
    def return_file_path_value(self, path, text, permittedTypes=[]):
        if len(path) == 0:
            ...
            return False
        if (
            len(permittedTypes) != 0
            and path.split(".")[-1].lower() not in permittedTypes
        ):
            self.set_text_color(text)
            return False
    ...
    def start_process(self):
        ...
        if self.p is None:  # No process running.
            ...
            self.message(
                ...
                + "/"
                + self.outputNameField.text()
            )
            self.message("Adapter File: " + self.adapterField.text())
            if self.variantCheckBox.isChecked():
                self.message("Variant setting enabled")
            if self.benchmarkCheckBox.isChecked():
                self.message("Benchmark setting enabled")
            self.message("\n\n")
            self.p = (
                QProcess()
