...
def remove_chimeras_from_umi_pairs(starcode1Path, starcode2Path, output, tdd=False):
    ...
    s1UMI, s1Indices = gather_umis_and_corresponding_indices_from_starcode(
        starcode1Path, tdd=tdd
    )
    s2UMI, s2Indices = gather_umis_and_corresponding_indices_from_starcode(
        starcode2Path, tdd=tdd
    )

    umiMatch1, umiMatch2, sharedIndices = sort_umi_pairs_by_number_of_matching_indices(
        s1UMI, s1Indices, s2UMI, s2Indices
    )
    umiMatch1, umiMatch2, sharedIndices = remove_duplicate_umis_from_pairs(
        umiMatch1, umiMatch2, sharedIndices
    )

    data = []
    for i in range(len(sharedIndices)):
        data.append(
            [
                umiMatch1[i] + umiMatch2[i],
                len(sharedIndices[i]),
                ",".join([str(x) for x in sorted(sharedIndices[i])]),
            ]
        )
...
def gather_umis_and_corresponding_indices_from_starcode(starcodePath, tdd=False):
    ...
    if isinstance(list(s1.iloc[:, 2])[0], int):
        raise Exception(
            "Fewer that 5 UMI clusters found with more than a single sequence"
        )
    s1UMI = s1.iloc[:, 0]
    ...
    if not tdd:
        s1UMI, s1Indices = [
            np.delete(np.array(x), (remove)) for x in [s1UMI, s1Indices]
        ]
        if len(s1Indices) < 5:
            raise Exception(
                "Fewer that 5 UMI clusters found with more than a single sequence"
...
def sort_umi_pairs_by_number_of_matching_indices(s1UMI, s1Indices, s2UMI, s2Indices):
    ...
    lengths, indicesList, umi1List, umi2List = zip(
        *sorted(zip(lengths, indicesList, umi1List, umi2List), reverse=True)
    )
...
def remove_duplicate_umis_from_pairs(umi1List, umi2List, indicesList):
    ...
    for i in range(len(indicesList)):
        umi1 = umi1List[i]
        umi2 = umi2List[i]
        if umi1 in umi1Set or umi2 in umi2Set:
            remove.append(i)
        else:
            umi1Set.add(umi1)
            umi2Set.add(umi2)
    indicesList, umi1List, umi2List = [
        np.delete(np.array(x), (remove)) for x in [indicesList, umi1List, umi2List]
    ]
...
def bin_sequences_by_umi_pair(seqPath, starcodePath, minReads):
    ...
    with open(seqPath) as handle:
        ...
        for record in SeqIO.parse(handle, "fastq"):
            index_recordID[count] = record.id
            count += 1
