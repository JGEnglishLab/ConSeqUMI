...
consensusFile = [
    outputDir + x for x in os.listdir(outputDir) if re.match("consensus.+\.fasta", x)
][0]
consDict = {
    int(record.id): str(record.seq).upper()
...
binFiles = sorted(
    [
        outputDir + "delete/" + x
        for x in os.listdir(outputDir + "delete/")
...
for binNum in sorted(consDict):
    ...
    tempDiffs = ps.find_all_diffs_between_candidate_and_binned_seqs(
        consDict[binNum], binDict[binNum]
    )
