...
class ReferenceConsensusGenerator:
    ...
    def justify_left_all_string_lengths_with_buffer(self, readSequences):
        ...
        justifiedSequences = [readSequence.ljust(maxLength + self.bufferLength) for readSequence in readSequences]
        return justifiedSequences

    def initialize_consensus_sequence_front(self, readSequences):
        readSequenceFronts = [readSequence[:self.sequenceWindowLength] for readSequence in readSequences]
    ...
    def generate_consensus_sequence(self, readSequences):
        justifiedReadSequences = self.justify_left_all_string_lengths_with_buffer(readSequences)
        consensusSequence = self.initialize_consensus_sequence_front(justifiedReadSequences)
        readSequenceLength = len(justifiedReadSequences[0])
        for i in range(readSequenceLength - self.bufferLength):
            readSequenceBufferWindows = [justifiedReadSequence[i : i + self.bufferLength] for justifiedReadSequence in justifiedReadSequences]
            consensusSequenceWindow = consensusSequence[-self.sequenceWindowLength:]
            nextCharacter = self.find_next_character_in_sequence(readSequenceBufferWindows, consensusSequenceWindow)
