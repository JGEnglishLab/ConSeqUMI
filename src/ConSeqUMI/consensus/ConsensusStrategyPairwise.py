...
from ConSeqUMI.consensus.consensusStrategyPairwiseFunctions import identify_differences_from_indices, find_in_string_indices_of_character, inject_difference_into_sequence
...
class ConsensusStrategyPairwise(ConsensusStrategy):
    ...
    def break_down_alignment_to_strings(self, alignment):
        ...
        if "target" in alignmentString:
            ...
            originalSequenceAlignment = "".join([targetLines[i][indices[i][0]:indices[i][1]] for i in range(len(indices))])
            indelIndicator = "".join([matchLines[i][indices[i][0]:indices[i][1]] for i in range(len(indices))])
            differentSequenceAlignment = "".join([queryLines[i][indices[i][0]:indices[i][1]] for i in range(len(indices))])
        else:
    ...
    def find_pairwise_score_and_all_differences_between_two_sequences(self, originalSequence, differentSequence):
        ...
        originalSequenceAlignment, indelIndicator, differentSequenceAlignment = self.break_down_alignment_to_strings(alignment)
