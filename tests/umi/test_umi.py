...
def test__umi__main(
    args, topUmi, bottomUmi, exampleForwardRecord, exampleReverseRecord
):
    umi.main(args)
    ...
    if targetSequenceRecordOutputs[0].id == "forward":
        (
            targetForwardRecordOutput,
            targetReverseRecordOutput,
        ) = targetSequenceRecordOutputs
    else:
        (
            targetReverseRecordOutput,
            targetForwardRecordOutput,
    ...
    assert targetForwardRecordOutput.letter_annotations["phred_quality"] == [
        40 for _ in range(200)
    ]
    assert len(targetReverseRecordOutput) == 200
    assert targetReverseRecordOutput.letter_annotations["phred_quality"] == [
        40 for _ in range(200)
    ]
    ...
    chimeraOutput = pd.read_csv(
        dataAnalysisPath + "chimera_summary_of_starcode_matches.csv"
    )
    assert list(chimeraOutput.columns) == [
        "top UMI",
        "bottom UMI",
        "Number of Reads",
        "Read Identifiers",
